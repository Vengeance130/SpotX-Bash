...
if [[ "${PLATFORM_TYPE}" == "macOS" ]]; then
  BD86="==geiRnL3cDNy0CZzkTN5QWN4cmL"
  BD64="==geiRnLwgDNy0CZzkTN5QWN4cmL"
...
if [[ "${UNINSTALL_FLAG}" == "true" ]]; then
  if [[ ! -f "${XPUI_BAK}" ]]; then
    ...
    if [[ "${PLATFORM_TYPE}" == "macOS" ]]; then codesign -f -s - --deep "${APP_PATH}" 2>/dev/null; fi
...
if [[ "${XPUI_SKIP}" == "false" ]]; then
  if [[ "${EXCLUDE_EXPERIMENTAL_FLAG}" == "true" ]]; then
    ...
    if [[ $(ver "${CLIENT_VERSION}") -ge $(ver "1.2.9.739") ]]; then $PERL "${ENABLE_YLX_Typeahead_Search}" "${XPUI_JS}"; fi
...
if [[ "${XPUI_SKIP}" == "false" ]]; then
  if [[ "${OLD_UI_FLAG}" == "false" && $(ver "${CLIENT_VERSION}") -ge $(ver "1.1.93.896") && "${EXCLUDE_EXPERIMENTAL_FLAG}" == "false" ]]; then
    ...
    if [[ "${DISABLERIGHTSIDEBAR_OPT}" == "true" ]]; then perl -e 'print "\xE2\x9C\x94\x20\x44\x69\x73\x61\x62\x6C\x65\x64\x20\x72\x69\x67\x68\x74\x20\x73\x69\x64\x65\x62\x61\x72\n"'; else if [[ $(ver "${CLIENT_VERSION}") -ge $(ver "1.1.98.683") ]]; then $PERL "${ENABLE_RIGHT_SIDEBAR}" "${XPUI_JS}"; if [[ $(ver "${CLIENT_VERSION}") -ge $(ver "1.2.7.1264") ]]; then $PERL "${ENABLE_RIGHT_SIDEBAR_CREDITS}" "${XPUI_JS}"; $PERL "${ENABLE_RIGHT_SIDEBAR_MERCH_FALLBACK}" "${XPUI_JS}"; $PERL "${ENABLE_RIGHT_SIDEBAR_TRANSITION_ANIMATIONS}" "${XPUI_JS}"; fi; fi; fi
